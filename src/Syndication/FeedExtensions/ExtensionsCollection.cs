...
public class ExtensionsCollection : List<ExtensionBase>
{
	...
	internal void AddNodesExtension(MLNode node)
	{
		foreach (ExtensionBase extension in this)
			if (extension is Yahoo.Data.YahooMedia yahoo)
				new Yahoo.Transforms.YahooMediaWriter().AddNodesExtension(node, yahoo);
			else if (extension is Desktop.Data.FeedDesktop feed)
				new Desktop.Transforms.FeedDesktopWriter().AddNodesExtension(node, feed);
	}
	...
	{
		MLNameSpacesCollection nameSpaces = new MLNameSpacesCollection();

			// Añade los espacios de nombres de las extensiones del canal
			foreach (ExtensionBase extension in channel.Extensions)
				nameSpaces.Add(extension.Prefix, extension.NameSpace);
