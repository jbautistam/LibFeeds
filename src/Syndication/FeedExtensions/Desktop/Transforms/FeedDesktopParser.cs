...
internal class FeedDesktopParser : ExtensionParserBase
{
	...
	{
		FeedDesktop channel = (FeedDesktop) objFeed.Extensions.Search(FeedDesktopConstTags.XMLDefaultNameSpace);
	...
	private void Parse(MLNode node, FeedDesktop channel)
	{
		if (node.Prefix.Equals(base.Prefix))
			switch (node.Name)
			{
				case FeedDesktopConstTags.Read:
						channel.IsRead = node.Value.GetBool(false);
					break;
				case FeedDesktopConstTags.Priority:
						channel.Priority = node.Value.GetInt(0);
					break;
				case FeedDesktopConstTags.Enabled:
