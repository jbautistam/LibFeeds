...
public class DesktopFilesWriter
{
	...
	/// </summary>
	public string? GetXML(DesktopFilesChannel channel) => GetFile(channel).ToString();
	...
	private MLFile GetFile(DesktopFilesChannel channel)
	{
		...
		MLNode node = file.Nodes.Add(DesktopFilesConstTags.Root);
			...
			node.Nodes.Add(DesktopFilesConstTags.DateCreated,
												DateTimeHelper.ToStringRfc822(channel.DateCreated));
			node.Nodes.Add(DesktopFilesConstTags.DateModified,
	...
	private void AddEntries(MLNode parent, DesktopFilesEntriesCollection entries)
	{
		...
		{
			MLNode node = parent.Nodes.Add(DesktopFilesConstTags.Outline);
				...
				if (entry.NumberNotRead > 0)
					node.Attributes.Add(DesktopFilesConstTags.NotRead, entry.NumberNotRead);
				if (!entry.Enabled)
					node.Attributes.Add(DesktopFilesConstTags.Enabled, entry.Enabled);
