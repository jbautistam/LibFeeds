...
namespace Bau.Libraries.LibFeeds.Process
{
	...
	public class Procesor
	{
			...
			AtomChannel channel = null;
				...
				try
				{
					string fileName = System.IO.Path.GetTempFileName();
						...
						// Descarga el archivo
						LibCommonHelper.Files.HelperFiles.SaveTextFile(fileName, await new HttpWebClient().HttpGetAsync(url));
						// Carga los datos
		...
		/// </summary>
		public AtomChannel Load(string fileName)
		{
			return Parse(new LibMarkupLanguage.Services.XML.XMLParser().Load(fileName));
		}

		/// <summary>
		///		Interpreta los datos de un texto XML
		/// </summary>
		public AtomChannel ParseText(string content)
		{
			return Parse(new LibMarkupLanguage.Services.XML.XMLParser().ParseText(content));
		}
